...
public class AulaVirtualController {
    ...
    //LOGIN:
    // http://localhost:8080/login/{teacher/student/admin}
    ...
    */
    ...
    public String mostrarPagPrincipal(){
        ...
        if(false/*(boolean)sesion.getAttribute("teacher")*/){
            //Si es profesor
            return "Profesor";
        }else if(true/*(boolean)sesion.getAttribute("student")*/){
            //Si es alumno
            return "Alumno";
        }else if(false/*(boolean)sesion.getAttribute("admin")*/){
            //No hay pagina principal para admin, se le redirige a gestionar alumnos
    ...
    @RequestMapping(value = "/secction", method = RequestMethod.GET)
    ...
    public String mostrarGestionAlumnos(@RequestParam(name = "edit", defaultValue = "false") String edit,
        ...
        if(edit.equalsIgnoreCase("true")){
            //Para editar
            //http://localhost:8080/student?edit=true (CRUD)
            ...
            if(!studentId.isEmpty()){
                ...
                id del alumno como parametro (http://localhost:8080/student?edit=true&student_id={id}).
    ...
    public String mostrarGestionCursos(@RequestParam(name = "edit", defaultValue = "false") String edit,
        ...
        //Verificar que este logueado y que sea administrador.
        if(edit.equalsIgnoreCase("true")){
            //http://localhost:8080/course?edit=true (CRUD)

            if(!courseID.isEmpty()){
                //http://localhost:8080/course?edit=true&course_id={id}
    ...
    public String mostrarGestionSecciones(@RequestParam(name = "edit", defaultValue = "false") String edit,
                                          ...
                                          Model model){
        //Verificar que este logueado -> Verificar que sea admin. Si es otro tipo de usuario se le podria redireccionar a la pagina principal.
        if(edit.equalsIgnoreCase("true")){
            //http://localhost:8080/section?edit=true (CRUD)

            if(!sectionID.isEmpty()){
                //http://localhost:8080/section?edit=true&section_id={id}
